...
Layer::Layer(int M, int N, int O)
{
	...
	std::vector<std::vector<float>> h_weight(N, std::vector<float>(M));
	...
	cudaMemcpy(bias, h_bias.data(), sizeof(float) * N, cudaMemcpyHostToDevice);

	// cudaMemcpy(weight, h_weight, sizeof(float) * M * N, cudaMemcpyHostToDevice);
	cudaMemcpy(weight, h_weight.data(), sizeof(float) * M * N, cudaMemcpyHostToDevice);
}
...
__global__ void fp_preact_c1(float input[28][28], float preact[6][24][24], float weight[6][5][5])
{
	...
	for (int n = N * pos / size; n < N * (pos+1) / size; ++n) {
		int idx = n;
		const int i1 = ((idx /= 1	) % 5);		// the row index within the 5x5 filter.
		const int i2 = ((idx /= 5	) % 5);		// the column index within the 5x5 filter.
		const int i3 = ((idx /= 5	) % 6);		// the index of the filter.
		const int i4 = ((idx /= 6	) % 24);	// the row index within the 24x24 output.
		const int i5 = ((idx /= 24	) % 24);	// the column index within the 24x24 output.
...
__global__ void fp_bias_c1(float preact[6][24][24], float bias[6])
{
	...
	for (int n = N * pos / size; n < N * (pos+1) / size; ++n) {
		int idx = n;
		const int i1 = ((idx /= 1	) % 6);		// the index of the filter.
		const int i2 = ((idx /= 6	) % 24);	// the row index within the 24x24 output.
		const int i3 = ((idx /= 24	) % 24);	// the column index within the 24x24 output.
...
__global__ void fp_preact_s1(float input[6][24][24], float preact[6][6][6], float weight[1][4][4])
{
	...
	for (int n = N * pos / size; n < N * (pos+1) / size; ++n) {
		int idx = n;
		const int i1 = ((idx /= 1	) % 4);		// the row index within the 4x4 filter.
		const int i2 = ((idx /= 4	) % 4);		// the column index within the 4x4 filter.
		const int i3 = ((idx /= 4	) % 6);		// the index of the filter.
		const int i4 = ((idx /= 6	) % 6);		// the row index within the 6x6 output.
		const int i5 = ((idx /= 6	) % 6);		// the column index within the 6x6 output.
