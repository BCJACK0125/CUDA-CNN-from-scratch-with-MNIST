...
static double forward_pass(double data[28][28])
{
	...
	// Set input layer
	// Size of output: 28x28
	l_input.setOutput((float *)input);
	
	// Convolution layer: kernel size 5x5, 6 kernels
	// Size of output: 24x24x6
	...
	apply_step_function<<<64, 64>>>(l_c1.preact, l_c1.output, l_c1.O);		// Sigma function

	// Subsampling layer: kernel size 4x4, stride 4
	// Size of output: 6x6x6
	...
	apply_step_function<<<64, 64>>>(l_s1.preact, l_s1.output, l_s1.O);		// Sigma function

	// Fully connected layer: 6x6x6 -> 10
	// Size of output: 10
...
static void learn()
{
	...
	while (iter < 0 || iter-- > 0) {
		...
		for (int i = 0; i < train_cnt; ++i) {
			...
			makeError<<<10, 1>>>(l_f.d_preact, l_f.output, train_set[i].label, 10);	// l_f.d_preact is assigned here
